...
Per-answer marginal cost, retry-adjusted:

- Batch ASR on a 3-minute answer: **~$0.013** (3 × $0.0043/min list)
- Critique call (rubric + transcript + metrics in, structured critique out): a single
  call lands around **~$0.15**, but thinking-token variance is high, `critiquer.py`
  allows two attempts, and the August reruns showed some answers needing both. A
  retry-adjusted expected critique (≈20% retry) is closer to **~$0.18** than the
  single-call ~$0.15.
- **~$0.19 per answer** (ASR ~$0.013 + ~$0.18). The whole-board Criterion 1 pass is
  one more call per board and is not in either number yet.
