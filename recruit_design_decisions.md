...
| **C3 — I has never been scored under the breadth clause** | It failed on both attempts in the set re-run, so the prediction that it holds at 5 is untested. Both failure paths are now fixed; re-run the whole set rather than I alone, since a second pass also gives the first repeat observation on every other answer. |
