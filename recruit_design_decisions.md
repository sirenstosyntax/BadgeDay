...
| **C3 — is a disposition that generalises the incident a *second* behaviour?** | **The surviving 1-of-20 on E, and Grant's to take.** The dissent no longer argues that an untied disposition cannot count — that is fixed — but that E's *"that's how it works"* is *"a general restatement of the same kind of help"*: one behaviour told twice. E's incident is helping a slower colleague; E's disposition is that helping slower colleagues is what you do. The breadth clause says the same behaviour told three times cannot clear it, and route 2 says a disposition supplies the second, and it does not say which governs when the disposition *is* the incident generalised. **Not fixed** — 1 of 20 is inside the harness's own unreadable band, closing it toward the dissent revises his blind 5, and today's repeated lesson is that a sentence aimed at a narrow gap moves what it was not aimed at. |
| **C3 — E has never once probed what it was written to probe** | Across **forty runs at n=20, twice**, no determination has decided on *"I've genuinely never had a problem with anyone."* The 5 sets it aside as *"a separate, still-open question about delivery risk"*; the 4B does not mention it. Noted after the first n=20 and now reproduced on the amended clause, which makes it a property of the fixture rather than of the rubric text. **This is not a null result about note 5** — the answer supplies two behaviours, so it is decided before the closing line is reached. Isolating the claim needs the same answer *without* a second behaviour, so the line is the only thing left to decide on. A fixture, not a clause. |
...
| **C3 — "one exchange does not supply two behaviours" is my call, not Grant's** | The judgment the ruling forced and the first thing to check on the re-run. F arguably shows two of anchor 5's behaviours already — he raises the difficult thing *and* he credits precisely — so a bare *"more than one behaviour"* clause would have left F at 5 and implemented nothing. I drew the clause to exclude two halves of a single conversation, which is what makes F a 4. **That line is the mechanism of the whole correction and Grant has not ruled on it.** If he reads F's asking-plus-crediting as genuinely two behaviours, the clause is wrong and the discriminator has to be something else — most likely conduct that outlasts the episode, which is what E and I both have and F does not. |
