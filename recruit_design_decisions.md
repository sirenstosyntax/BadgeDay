...
| **Zero drift across nine answers between two set runs** | The first repeat observation on this harness, and the only way to separate a rubric finding from pipeline noise on a one-call-per-answer path. Every answer that scored in run 1 returned the same value in run 2. Worth holding onto as the baseline: after this, an answer that moves between runs is a pipeline question, not a clause question. |
