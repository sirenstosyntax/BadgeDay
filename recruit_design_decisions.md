...
| **The whole review set needs re-running, not just F** | The breadth clause raises the bar for **every** answer, so the re-run is `scripts/recruit_review_compare.py` across all eleven rather than `recruit_answer_runs.py F`. Reading the amended anchor by hand against the transcripts: **F should fall to 4** (one exchange, nothing else), **E should hold at 5** (teaching a newer lad *plus* reciprocity described as how it works, across three years), **I should hold at 5** (changed by a named colleague, took the correction without distributing it, and the change outlasted the incident — *"what I do differently now"*). If E or I falls, the clause is over-tightened; if F holds at 5, it did not bite. Either would be the same class of defect as the edit it corrects, from the other direction. |
