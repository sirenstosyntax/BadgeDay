...
- **Question bank variant pass 2 complete 2026-09-10 (Grant):** 16 struck, **272 survive**. Flagged-family strikes: JOB-3 items 1, 2, 8 (prep-only); JUD-4 item 3 (engine vs prevention funding); CMT-2 item 6; CON-2 items 6, 8; SLF-3 items 5, 8. Batch-skim exceptions (Red accepted; Grant accepted all seven): INT-4.1, INT-4.4, STR-3.6, JOB-1.2, JOB-2.6, JOB-4.5, OPN-2.6. All other items KEEP. Review of families and variants is complete. Publishing the bank into product is a separate start order — nothing publishes until Grant opens publish. Does not wire the live scorer or pricing.
- **C2-only bank publish 2026-09-10 (Grant start order):** 81 issuable MOT/JOB/CMT prompts (non-STRIKE) are the live bank. Hold issuing C1/C3/C4/C5 until scorer wire. Hold full board of five, family-within-8, 70/30, retirement. Legacy `scenario_id=c2` is frozen and aliased by exact wording so novelty stays honest.
- Five-question board *build* and live scorer wire remain held — implementation hold, not a rubric hold. Session shape is settled; implementation is not started.
