...
- Declared plugins already in `mobile/ios/package.json`: `@capacitor/filesystem`, `@capacitor/camera`, `@capacitor/local-notifications`, `@capgo/native-purchases` (and core/app/splash). StoreKit bridge **is declared** as `@capgo/native-purchases` (PR 79); still wire purchase → `POST /billing/store/appstore/purchase` per AC 20–25. IAP product create remains **HELD**; do not invent `APPSTORE_PRODUCT_ID_*`.
