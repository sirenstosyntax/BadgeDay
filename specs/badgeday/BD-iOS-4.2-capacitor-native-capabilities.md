...
- Declared plugins already in `mobile/ios/package.json`: `@capacitor/filesystem`, `@capacitor/camera`, `@capacitor/local-notifications`, `@capgo/native-purchases` (and core/app/splash). The StoreKit bridge **is** declared (`@capgo/native-purchases`); landed in web + plugin declare. IAP product create remains **HELD**. Do not invent `APPSTORE_PRODUCT_ID_*`.
